...
scripts/train_v5.py
───────────────────
CIELO·TUC v5.0 Training — Multi-zone (10 zones) + Synthetic Events Across All Zones
+ Horizon-weighted + Enhanced Features + Temporal CV + Per-Zone Breakdown

Key improvements over v4.0:
  1. 10 zones across Tucumán (up from 5) — Capital Norte, Bella Vista, Lules,
     Monteros, Chicligasta added
  2. Synthetic events distributed across ALL zones (critical fix) — v4.0 only
     added them to Capital, causing 0% precision for zonda/storm/hail globally
  3. 20 years of NASA POWER data (up from 15), 3000 synthetic events (up from 2000)
  4. New event types: cold_front (10%), multi_day_storm (5%)
  5. Gaussian bell curve intensity profiles instead of sine
  6. Zone-specific base temperatures based on altitude
  7. Per-zone evaluation breakdown in final results
  8. Deeper model: ~250K params (CNN 64, LSTM 112, Head 56)

Usage:
  python scripts/train_v5.py --years 20 --n-synthetic 3000 --max-epochs 100 --folds 3 --batch-size 512
"""
...
class FastWeatherModelV5(nn.Module):
    ...
    def __init__(self, n_features: int, n_timesteps: int = LOOKBACK, horizons: list = None):
        ...
        # Single CNN layer — 64 channels (was 48)
        ...
        # 2-layer LSTM — 112 hidden (was 96)
        self.lstm = nn.LSTM(
            input_size=64,
        ...
        # Per-horizon heads — 56 units (was 48)
...
def train_v5_fold(
    ...
    """Train a single fold of the temporal CV using FastWeatherModelV5."""
...
async def main(
    ...
    logger.info("=" * 60)
    logger.info("CIELO·TUC v5.0 Training")
    logger.info("  10-Zone + Synthetic Events All Zones + Horizon-Weighted")
    logger.info("  Enhanced Features + Temporal CV + Per-Zone Breakdown")
    ...
    # ── Save model ──
    version = f"5.0-{datetime.now(timezone.utc).strftime('%Y%m%d')}"
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Train CIELO·TUC v5.0")
