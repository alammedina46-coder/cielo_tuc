...
scripts/train_v4.py
───────────────────
CIELO·TUC v4.0 Training — Multi-zone + Horizon-weighted + Enhanced Features + Temporal CV

Key improvements over v2.0/v3.0:
  1. Multi-zone training data — 5 points across Tucumán province (~5x data)
  2. Horizon-weighted loss — short horizons weighted higher
  3. Temperature seasonal normalization — RMSE as % of 35° seasonal range
  4. Better synthetic events — pre-storm buildup, mixed events, autocorrelated noise
  5. Enhanced features — 20 new features (14 existing + 6 new)
  6. Zone-aware evaluation — train all zones, evaluate on capital only
  7. 3-fold temporal cross-validation
  8. Huber loss delta=5.0 + directional accuracy for temperature

Usage:
  python scripts/train_v4.py [--years 15] [--n-synthetic 2000] [--max-epochs 100] [--folds 3] [--batch-size 512]
"""
...
# ── Model ───────────────────────────────────────────────────────
class FastWeatherModelV4(nn.Module):
    """
    v4.0 model — same architecture as v2.0 FastWeatherModel but with 52 input features.
    2-layer CNN + 2-layer LSTM + Attention + per-horizon heads.
    ~170K parameters.
    """
...
async def download_all_zones(years_back: int) -> Dict[int, pd.DataFrame]:
    """Download NASA POWER for all 5 zones. Returns dict of zone_id → DataFrame."""
    logger.info(f"Downloading NASA POWER for {len(ZONE_POINTS)} zones ({years_back} years)...")
    dfs = {}
    for i, zp in enumerate(ZONE_POINTS):
        logger.info(f"  [{i+1}/5] {zp['zone_name']} ({zp['lat']}, {zp['lng']})...")
...
# ── Improved Synthetic Events (v4) ─────────────────────────────
def generate_synthetic_extremes_v4(n_events: int = 2000) -> pd.DataFrame:
    """
    ...
    - More realistic ranges from actual Tucumán climate data.
    """
    logger.info(f"Generating {n_events} synthetic extreme events (v4)")
    rows = []
    ...
    for _ in range(n_events):
        ...
        for h in range(total_h):
            ...
            if event_type == "zonda":
                intensity = np.sin(p * np.pi) if phase == "main" else (
                    0.3 * p if phase == "buildup" else 0.3 * (1 - p)
                )
                if is_mixed and phase == "main" and p > 0.6:
                    ...
                    row = {
                        "timestamp": t,
                        "temperature_c": (28 + 17 * intensity * (1 - storm_t) - 5 * storm_t
                                          + noise_temp),
                    ...
                    row = {
                        "timestamp": t,
                        "temperature_c": 28 + 17 * intensity + noise_temp,
                ...
                intensity = np.sin(p * np.pi) if phase == "main" else (
                    0.2 * p if phase == "buildup" else 0.2 * (1 - p)
                )
                row = {
                    "timestamp": t,
                    "temperature_c": 25 - 5 * intensity + noise_temp,
                ...
                intensity = np.sin(p * np.pi) if phase == "main" else (
                    0.4 * p if phase == "buildup" else 0.4 * (1 - p)
                )
                row = {
                    "timestamp": t,
                ...
                intensity = np.sin(p * np.pi) if phase == "main" else (
                    0.15 * p if phase == "buildup" else 0.15 * (1 - p)
                )
                row = {
                    "timestamp": t,
                    "temperature_c": 24 - 4 * intensity + noise_temp,
...
# ── Enhanced Features v4 (20 new features) ─────────────────────
...
def build_tensors_multi_zone(
    ...
    Each zone's data gets a zone_id column (0-4).
...
def train_v4_fold(
    ...
    """Train a single fold of the temporal CV."""
    model = FastWeatherModelV4(
...
async def main(
    ...
    logger.info("=" * 60)
    logger.info("CIELO·TUC v4.0 Training")
    logger.info("  Multi-zone + Horizon-weighted + Enhanced Features + Temporal CV")
    ...
    # ── 1. Download multi-zone data ──
    logger.info("\n[1/6] Downloading NASA POWER data (5 zones)...")
    t0 = time.time()
    ...
    logger.info(f"\n[2/6] Generating {n_synthetic} synthetic events (v4)...")
    df_synth = generate_synthetic_extremes_v4(n_events=n_synthetic)
    # Add synthetic data to capital zone (zone 0) if available
    if 0 in dfs_by_zone:
        dfs_by_zone[0] = pd.concat(
            [dfs_by_zone[0], df_synth], ignore_index=True
        ).sort_index()
        logger.info(f"  Synthetic events added to Capital zone")

    # ── 3. Build multi-zone tensors ──
    logger.info(f"\n[3/6] Building multi-zone tensors (lookback={LOOKBACK}h)...")
    ...
    logger.info(f"\n[4/6] Temporal {n_folds}-Fold Cross-Validation...")
    ...
    for fold_idx, (train_idx, val_idx) in enumerate(folds):
        ...
        # Check if validation has capital zone data
    ...
    logger.info(f"\n[5/6] Aggregating {n_folds}-fold results...")
    avg_metrics = {}
    for h in HORIZONS:
        hk = f"{h}h"
        avg_m = {}
    ...
    # Zone-aware evaluation: evaluate best model on capital zone (zone 0) only
    capital_mask = zone_ids == 0
    X_capital = X[capital_mask]
    y_capital = {h: v[capital_mask] for h, v in y.items()}
    logger.info(f"\n  Zone-aware evaluation (Capital only): {capital_mask.sum():,} samples")

    capital_preds, capital_tgts = collect_preds(
        best_model, X_capital, y_capital, HORIZONS, device
    )
    capital_metrics = evaluate_v4(capital_preds, capital_tgts, HORIZONS, HORIZON_WEIGHTS)
    ...
    for h in HORIZONS:
        ...
        logger.info(
            f"    Capital-zone composite: {cm['composite_accuracy']:.4f}"
        )
    ...
    capital_weighted = sum(
        HORIZON_WEIGHTS[h] * capital_metrics[f"{h}h"]["composite_accuracy"]
        for h in HORIZONS
    ) / weight_sum

    logger.info(f"\n  {'='*40}")
    logger.info(f"  WEIGHTED COMPOSITE:     {weighted_composite:.4f} ({weighted_composite*100:.1f}%)")
    logger.info(f"  CAPITAL-ZONE COMPOSITE: {capital_weighted:.4f} ({capital_weighted*100:.1f}%)")
    ...
    # ── 7. Save model ──
    version = f"4.0-{datetime.now(timezone.utc).strftime('%Y%m%d')}"
    ...
    logger.info(f"  Weighted composite: {weighted_composite:.4f} ({weighted_composite*100:.1f}%)")
    logger.info(f"  Capital-zone:       {capital_weighted:.4f} ({capital_weighted*100:.1f}%)")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Train CIELO·TUC v4.0")
